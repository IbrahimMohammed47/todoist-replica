...
public class ViewSortedBoardCommand extends Command {
	...
	public void execute() throws Exception {
		try {
			...
			String Id = body.getString("boardId");

			ObjectId mongoId = new ObjectId(Id);

			String cachedBoard = "boardTasks:" + Id;
			ArrayList<Document> tasks = new ArrayList<Document>();

			Document query = new Document("_id", mongoId);
			Document query2 = new Document("boardId", Id);

			Document board = null;

			for (Document x : database.getCollection("boards").find(query)) {
				board = x;
			}
			...
			if (cacheObject.exists(cachedBoard)) {
				...
				Set<String> tasksIds = cacheObject.zrange(cachedBoard, 0, -1);

				if (tasksDocCount != tasksIds.size()) {
					cacheObject.zremrangeByRank(cachedBoard, 0, -1);
					populateCacheEntity(query2, cacheObject, cachedBoard, database, tasks);

				} else {

					populateTasksResponseFromCache(tasksIds, cacheObject, tasks, cachedBoard);
