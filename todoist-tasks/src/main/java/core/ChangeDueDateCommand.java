...
public class ChangeDueDateCommand extends Command {
...
//cache
			...
			if (cacheObject.exists(tasksCacheNotation))
				cacheObject.hset(tasksCacheNotation, "dueDate", dueDate);
			else {
				Document documentToBeCached = database.getCollection("tasks").find(new Document("_id", taskId)).first();
				populateCacheEntity(documentToBeCached, cacheObject, database);
			}

			JSONObject res = new JSONObject();
			res.put("new Due Date", dueDate);
